...

{
  environment.systemPackages = with pkgs; [
    ...
    (pkgs.writeShellScriptBin "compressall" ''
      ...
      # 1. Setup Folder Names
      current_folder_name=$(basename "$PWD")
      parent_dir=$(dirname "$PWD")
      old_folder_name="''${current_folder_name} - OLD"

      # Create the temporary old directory
      mkdir -p "$old_folder_name"

      # 2. Find Files (Using "" and ''${} for Nix compatibility)
      ...
      if [ ''$total_files -eq 0 ]; then
        ...
        rmdir "$old_folder_name" 2>/dev/null
        exit 1
      fi

      show_progress_bar() {
        local duration=''${1%.*} # Handle potential floats from ffprobe
        local elapsed=''${2%.*}
      ...
      for input in "''${files[@]}"; do
        ...
        if [[ "''$extension_lower" == "mp4" || "''$extension_lower" == "mov" || "''$extension_lower" == "avi" || "''$extension_lower" == "mkv" || "''$extension_lower" == "3gp" || "''$extension_lower" == "webm" || "''$extension_lower" == "flv" || "''$extension_lower" == "wmv" || "''$extension_lower" == "m4v" || "''$extension_lower" == "mpeg" || "''$extension_lower" == "mpg" || "''$extension_lower" == "divx" ]]; then
          duration=$(ffprobe -i "''$input" -show_entries format=duration -v quiet -of csv="p=0" 2>/dev/null | awk '{print $1}')
        ...
        if [ -f "''$output_file" ] && [ -s "''$output_file" ]; then
          ...
          if [ ''$output_size -ge ''$input_size ]; then
            ...
            # Move to the local OLD folder for now
            mv "''$input" "''$old_folder_name/"
            compressed_size=$((compressed_size + output_size))
          fi
        fi
      done

      # 4. Finalizing and Renaming
      if [[ ! "''$current_folder_name" =~ \ -\ COMP$ ]]; then
        ...
        if mv "''$current_folder_name" "''$new_dir_name" 2>/dev/null; then
          echo -e "\nFolder renamed to: ''$new_dir_name"
          # Update the path to the OLD folder inside the newly named parent
          old_folder_path="''${parent_dir}/''${new_dir_name}/''${old_folder_name}"
        else
          old_folder_path="''${parent_dir}/''${current_folder_name}/''${old_folder_name}"
        fi
      fi

      # 5. THE TRASH STEP (Only happens if we reached the end)
      if [ -d "''$old_folder_path" ]; then
        echo "Processing complete. Trashing originals... 🗑️"
        trash-put "''$old_folder_path"
        echo "Done!"
      fi
    '')
  ];
